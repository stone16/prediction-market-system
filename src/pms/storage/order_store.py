            ...
            async with connection.transaction():
                await connection.execute(
                    ...
                    SET market_id = EXCLUDED.market_id,
                        ...
                        pre_submit_quote_json = EXCLUDED.pre_submit_quote_json,
                    ...
                    order.outcome,
                    order.time_in_force,
                    json.dumps(dict(order.pre_submit_quote)),
                    order.intent_key,
...
def _order_from_row(row: asyncpg.Record) -> OrderState:
    ...
    return OrderState(
        ...
        status=cast(str, row["status"] or payload["status"]),
        market_id=cast(str, row["market_id"]),
        token_id=cast(str | None, row["token_id"] or payload.get("token_id")),
        venue=cast(Venue, row["venue"] or payload["venue"]),
        ...
        raw_status=cast(str, row["raw_status"] or payload["raw_status"]),
        ...
        pre_submit_quote=cast(
            dict[str, Any],
            _json_object(row["pre_submit_quote_json"])
            if row["pre_submit_quote_json"] is not None
            else payload.get("pre_submit_quote", {}),
        ),
        action=cast(str | None, row["action"] or payload.get("action")),
