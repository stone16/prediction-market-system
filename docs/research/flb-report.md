...
> Historical Gamma fallback report from 2026-05-03. This predates the
> runtime signal-specific calibration gate and is not a launch artifact.
> Launch readiness requires a strict warehouse CSV plus
> `/secure/pms/flb-calibration.csv` generated by
> `scripts/flb_data_feasibility.py --source warehouse-csv --calibration-csv`.
...
**H1 NOT VIABLE YET.** Insufficient resolved markets in runtime calibration signal buckets. Collect more strict warehouse data before proceeding with FLB strategy.
...
2. **Data source:** Gamma API `closed=true` returns a small window of recently resolved markets. For launch readiness, Dune Analytics on-chain data or a historical warehouse must provide ≥100 original market rows in each runtime FLB signal bucket.
