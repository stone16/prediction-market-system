    ...
    resolved = resolve_alembic_database_url(
        {
            ...
            "PMS_DATABASE__DSN": "postgresql://localhost/pms_nested",
...
def test_resolve_alembic_database_url_falls_back_to_nested_database_dsn() -> None:
    resolved = resolve_alembic_database_url(
        {
            "PMS_DATABASE__DSN": "postgresql://localhost/pms_nested",
            "PMS_DATABASE_URL": "postgresql://localhost/pms_fallback",
    ...
    assert resolved == "postgresql+psycopg://localhost/pms_nested"
...
def test_resolve_alembic_database_url_skips_empty_primary_url() -> None:
    resolved = resolve_alembic_database_url(
        {
            "DATABASE_URL": "",
            "PMS_DATABASE_URL": "postgresql://localhost/pms_fallback",
...
def test_resolve_alembic_database_url_requires_explicit_env_var() -> None:
    ...
    assert "PMS_DATABASE__DSN" in str(exc_info.value)
